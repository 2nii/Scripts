...
class TcpHandshake(object):
    ...
    def send_syn(self):
        ...
        response = sr1(self.l4)  #self._sr1(self.l4)
        self.l4[TCP].seq += 1
    ...
    def send_cortex_data(self):
        self.l4[TCP].flags = "PA"
        response = sr1(self.l4/cortex())
        self.seq_next = self.l4[TCP].seq + 1
        self.l4[TCP].seq += 1
        return self.handle_recv(response)
...
if __name__=='__main__':
    ...
    conf.verb = 0
    tcp_hs = TcpHandshake(("10.10.9.100",8000))
    tcp_hs.start()
    #print repr(tcp_hs.send_data("INTENTIONAL BAD REQUEST\r\n\r\n\r\n"))
    repr(tcp_hs.send_data(""))
    tcp_hs.send_cortex_data()
    tcp_hs.send_fin()
